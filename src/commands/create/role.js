...
module.exports = function registerRole(create) {
  create
    ...
    .action((name, keys) => {
      ...
      loadKubernetesResourceDefault(resourceGenerator.role(values))
        .then((res) => console.log(tableGenerator.roleSuccessTable(res.body)))
        .catch(handleError);
    })
    .addHelpText('after', `
  Example:
    $ ksql create role <name> "(namespace default, get deployments services)"
`);
};
